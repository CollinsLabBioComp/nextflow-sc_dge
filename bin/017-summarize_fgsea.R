...
command_line_interface <- function() {
    ...
    parser <- optparse::OptionParser(
        ...
        description = paste0(
            "Summarizes similarity of fgsea terms."
        )
    )
...
run_analysis <- function(param) {
    ...
    for (term_summary_type_i in term_summary_types) {
        ...
        for (j in unique(df_gsea_i$df_key)) {
            ...
            # add the plot data to the master data list
            ...
            reducedTerms$term <- unlist(lapply(
                reducedTerms$term_id,
                FUN = function(x) {
                    stringr::str_squish(tm::removeWords(
                        stringr::str_replace_all(
                            tolower(paste(
                                x,
                                sep = " ",
            ...
            reducedTerms$parentTerm <- unlist(lapply(
...
dev <- function() {
    ...
    param[["gsea_results"]] <- fs::path_expand(
    ...
    param[["gsets_gene_matrix"]] <- fs::path_expand(
